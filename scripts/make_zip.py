...
def  main():
  ...
  installer = "\installer\Disthorzion Installer.exe"
   
  if demo:
    installer = "\installer\Disthorzion Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Disthorzion manual.pdf" 
  ]
  ...
  ZIPNAME = "Disthorzion-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Disthorzion-v" + FULLVERSIONSTR + "-win-demo.zip"
